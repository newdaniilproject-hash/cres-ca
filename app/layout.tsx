...
export const viewport: Viewport = {
  ...
  // здесь и в CLAUDE.md («зум не блокируем, это нарушение
  // доступности») — не возвращать её обратно.
  //
  // Причина: продукт открывается в обёртке Capacitor и должен вести
  // себя как приложение. Расщипнутый пальцем экран в приложении
  // читается как поломка — в нативных экранах такого не бывает,
  // а обратно человек его не всегда сводит.
  //
  // Что это НЕ отменяет: пол в 16px у полей ввода остаётся
  // обязательным. Мобильный Safari `user-scalable=no` ИГНОРИРУЕТ
  // (с iOS 10), и без этого пола он по-прежнему зумил бы страницу
  // на фокусе поля и не отъезжал обратно. Меру снимает только
  // размер шрифта, а не эта настройка.
  maximumScale: 1,
  userScalable: false,
}
    ...
    <html lang="uk" className={inter.variable} suppressHydrationWarning>
      ...
      <body>
        ...
        <ToastProvider overlay={<><NativeProvider /><PwaProvider /><OfflineBar /></>}>
          ...
          <DeepLink />
          {/* ОБЁРТКА СТРАНИЦЫ. Нужна ровно для одного — чтобы под жестом
              «назад» можно было показать ПРЕДЫДУЩИЙ экран (свайп как
              в инстаграме, решение владельца 19.08.2026).

              Раньше жест двигал сам `body`, и под ним был фон документа,
              то есть пустота. Подложке со снимком предыдущей страницы
              нужно место ПОД содержимым и вне того, что едет за пальцем;
              с `body` такого места нет — оно и есть содержимое.

              Разбор целиком — `components/swipe-back.tsx`. Шторки
              и уведомления сюда намеренно не входят: они уходят порталом
              в `body` и остаются на месте. */}
