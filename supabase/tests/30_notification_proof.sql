...
-- ⚠️ ЩО ЦЕЙ БЛОК ПЕРЕВІРЯЄ, А ЩО НІ. Знайдено аудитом 25.08.2026.
--
-- Він перевіряє функцію `compliance_report()` — і вона працює. Але
-- ДОКУМЕНТ, який реально несуть на перевірку, збирає не вона, а роут
-- `app/app/journals/report/route.ts`, і цю функцію він не викликає
-- ЖОДНОГО разу. Тобто зелений результат нижче доводить, що правильно
-- працює гілка, яка в бою не виконується.
--
-- Саме тому 0107 (стан підтвердження + «непідтверджені зверху») пішла
-- у функцію і НЕ доїхала до паперу: тест був зелений, а в документі
-- колонки не було. Роут виправлено 25.08.2026; функція лишилась —
-- рішення про її долю за власником (правило 8 каже видаляти мертве).
--
-- Правило проекту, яке тут порушено і назване вголос: «тест зобовʼязаний
-- звати те саме, що зве бій». З SQL роут не викликати, тому покриття
-- документа лишається за рендером, а не за цим файлом.
\echo '--- 0106: compliance_report() (УВАГА: не той шлях, яким збирається документ)'
