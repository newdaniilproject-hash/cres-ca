...
LOG="$(mktemp)"
{
  ...
  # 25 стоит ПЕРЕД 24, и это не опечатка в порядке: он тоже обёрнут
  # в begin/rollback и месту безразличен, а 24 запускает уборку по сроку
  # и обязан оставаться последним (см. ниже). Номер отражает время
  # появления файла, порядок — то, что после 24 ставить нельзя ничего.
  psql -f "$ROOT/supabase/tests/25_staff_card.sql"
  psql -f "$ROOT/supabase/tests/26_tenant_export.sql"
  psql -f "$ROOT/supabase/tests/27_returns.sql"
  psql -f "$ROOT/supabase/tests/28_reviews.sql"
  psql -f "$ROOT/supabase/tests/29_attribution.sql"
  psql -f "$ROOT/supabase/tests/30_notification_proof.sql"
  psql -f "$ROOT/supabase/tests/31_profile_theme.sql"
  psql -f "$ROOT/supabase/tests/32_module_registry.sql"
  psql -f "$ROOT/supabase/tests/33_material_photo.sql"
  psql -f "$ROOT/supabase/tests/34_receipt_costing.sql"
  psql -f "$ROOT/supabase/tests/35_relocation.sql"
  psql -f "$ROOT/supabase/tests/36_order_ships_stock.sql"
  psql -f "$ROOT/supabase/tests/37_reorder_digest.sql"
  psql -f "$ROOT/supabase/tests/38_email_change_sync.sql"
  ...
  psql -f "$ROOT/supabase/tests/43_presets.sql"
  psql -f "$ROOT/supabase/tests/44_notification_dismiss.sql"
  psql -f "$ROOT/supabase/tests/45_journal_idem.sql"
  psql -f "$ROOT/supabase/tests/46_notification_settings.sql"
  psql -f "$ROOT/supabase/tests/47_avatar.sql"
  psql -f "$ROOT/supabase/tests/48_tenant_status.sql"
  psql -f "$ROOT/supabase/tests/49_live.sql"
  psql -f "$ROOT/supabase/tests/50_remove.sql"
  ...
  psql -f "$ROOT/supabase/tests/24_step7.sql"
} 2>&1 | tee "$LOG"
