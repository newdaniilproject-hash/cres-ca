...
Настройки аккаунтов, которые делаются один раз руками, и тексты
для карточек магазинов. Сборка идёт в облаке Codemagic — Mac не нужен.

## Чего здесь БОЛЬШЕ НЕТ

Раньше этот файл описывал сборку нативной обёртки Capacitor вокруг
боевого сайта: бинарь без нашего кода, Face ID через мост, пуши
OneSignal, теги `ios-v*` / `android-v*` / `play-v*`. Этого пайплайна
больше не существует — обёртка отменена 05.08.2026 и удалена из
репозитория вместе с `codemagic.yaml` и patch-скриптами
(CLAUDE.md → «Мобильная версия», правило 8).

**Приложение теперь — Flutter, в папке `mobile/`.** Бандл тот же
(`com.cresca.app`), поэтому всё, что настроено ниже — сертификаты,
записи в App Store Connect и Play Console, Firebase, OneSignal, —
переносится как есть и заново не заводится.

**Пайплайна сборки пока нет.** Он пишется заново под Flutter
(модуль М16); до тех пор сборка — локальная (`flutter build`).
Мины, оплаченные на DaKi и обёртке (постоянный ключ подписи,
entitlements на таргете, счётчик версий, perl-замены), сохранены
в КОНСПЕКТЫ.md, М16 — они по большей части про Codemagic и Apple,
а не про Capacitor, и пригодятся снова.
...
1. Сборка приезжает в App Store Connect (пайплайн под Flutter пишется,
   пока — `flutter build ipa` и Transporter).
...
4. Ревью 1–3 дня → Release. И здесь важное отличие от обёртки:
   теперь КАЖДОЕ изменение приложения требует новой сборки и ревью —
   раньше правки прилетали с git push мгновенно. Это осознанная плата
   за офлайн, который обёртка не давала.
