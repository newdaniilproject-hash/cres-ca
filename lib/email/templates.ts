...
// ── Смена почты: подтверждение ссылкой ───────────────────────────────
// ЕДИНСТВЕННОЕ письмо Supabase, где остаётся ССЫЛКА, а не код, — и это
// решение, а не недосмотр. У смены почты подтверждения ДВА (старый и
// новый адрес, Secure email change), а `verifyOtp` с кодом принимает
// один; экран с двумя полями кода для двух почт запутал бы сильнее,
// чем два клика. Ломкость ссылок здесь терпима: человек уже в аккаунте,
// сессия при переходе не нужна — GoTrue подтверждает токеном из адреса
// и приводит на /auth/confirm.
// Вставляется руками: Authentication → Emails → Change Email Address.
// Вместо аргумента в дашборде стоит {{ .ConfirmationURL }}.
export function mailEmailChange(confirmUrl: string): Mail {
  return {
    subject: 'Підтвердіть зміну пошти — CRES-CA',
    html: emailLayout({
      preheader: 'Потрібно підтвердити і старий, і новий адрес.',
      heading: 'Зміна пошти',
      body:
        P('Ви змінюєте адресу входу в CRES-CA. Натисніть кнопку, щоб підтвердити:') +
        P(`<a href="${confirmUrl}" style="display:inline-block;padding:12px 22px;` +
          `border-radius:10px;background:${MAIL.accent};color:#ffffff;` +
          `font:600 15px/1 -apple-system,Arial,sans-serif;text-decoration:none;">` +
          'Підтвердити зміну</a>') +
        P('Такий самий лист прийшов і на другу адресу — підтвердіть обидва. ' +
          'Поки не підтверджені обидва, вхід працює за старою поштою.'),
      footNote:
        'Якщо ви не змінювали пошту — не натискайте кнопку і змініть пароль: ' +
        'хтось отримав доступ до вашого акаунта.',
    }),
  }
}
