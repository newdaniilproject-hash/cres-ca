...
if [ -f "$MANIFEST" ]; then
  ...
  # ── ТОЛЬКО ВЕРТИКАЛЬНО ─────────────────────────────────────────────────
  # Решение владельца 25.08.2026: «удали возможность поворота экрана, наше
  # приложение только вертикальное на телефоне».
  #
  # `screenOrientation="portrait"`, а НЕ `sensorPortrait` и не `user`:
  # первое разрешает переворот «вверх ногами», второе слушается системной
  # настройки автоповорота, то есть поворот всё равно возможен.
  #
  # Второе условие обязательно и без него правка бессмысленна:
  # `configChanges` уже содержит `orientation|screenSize` (их кладёт сам
  # Capacitor). Без блокировки активность просто ПЕРЕЖИВАЛА поворот,
  # не пересоздаваясь, — то есть страница не перезагружалась, но ложилась
  # набок. Здесь чинится именно поворот, а не пересоздание.
  if ! grep -q 'android:screenOrientation' "$MANIFEST"; then
    perl -0pi -e 's/(<activity\b)/$1\n      android:screenOrientation="portrait"/' "$MANIFEST"
    echo "OK: AndroidManifest — только вертикальная ориентация"
  fi
