...
function AppShellInner({
  ...
  //
  // Отзыв владельца 30.08.2026: «когда я переключаюсь на разные экраны,
  // показывается не начало/верх экрана, а чуть ниже».
  //
  // Next при переходе не ставит прокрутку в ноль, а показывает начало
  // ИЗМЕНИВШЕГОСЯ сегмента — и упирает его верх в верх окна. Над ним
  // при этом стоит липкая шапка, поэтому первые её пикселей полсотни
  // накрывают начало содержимого: экран открывается «чуть ниже».
  //
  // ⚠️ ТОЛЬКО НА ПЕРЕХОДАХ ВПЕРЁД. На «назад» человек ожидает вернуться
  // РОВНО туда, где был, — в середину списка, из которого он ушёл
  // в карточку; безусловный ноль отнял бы это на каждом возврате и был
  // бы дефектом хуже исходного. Отличаем по `popstate`: он приходит
  // до перерисовки, поэтому флага хватает и таймеров не нужно.
