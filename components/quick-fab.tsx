...
export type QuickAction = {
  ...
  /**
   * ⚠️ ТОНА У ПУНКТА НЕТ И НЕ ЗАВОДИТЬ. Он здесь был дважды и оба раза
   * снимался по отзыву владельца: сперва пять тонов на пять пунктов
   * («ёлка»), потом акцент у одного главного — «почему стиль кнопки
   * Засіб отличается от других» (25.08.2026).
   *
   * Причина глубже вкуса: цвет в этом продукте означает СОСТОЯНИЕ,
   * а у пункта меню состояния нет. Плюс синий кружок в двух
   * сантиметрах от синей кнопки с плюсом — это один знак дважды.
   * Различают пункты подписи, и они обязательны у каждого.
   */
  href?: string
  onClick?: () => void
      ...
      {open && (
        <div className="fab-stack lg:hidden" role="menu">
          {actions.map((a, i) => {
            ...
            const inner = (
              <>
                <span aria-hidden className="fab-item-icon">
                  <a.icon size={16} />
                </span>
                {a.label}
              </>
